...
function main() {
    ...
    bodies.push(new Body(5.97e30, new Vector((400 + 92) * DIST_SCALE, 400 * DIST_SCALE), new Vector(0, 0)))
    // moon ?!
    bodies.push(new Body(7.35e22, new Vector((400 + 92 + 131.414) * DIST_SCALE, 400 * DIST_SCALE), new Vector(0, 0 + 13575630)))
    ...
    const render = () => {
        ...
        for(let i = 0; i < bodies.length; ++i) {
            bodies[i].integrate()
            ctx.beginPath()
            ctx.arc(bodies[i].pos.x / DIST_SCALE, bodies[i].pos.y / DIST_SCALE, .5, 0, 2 * Math.PI)
            ctx.fill()
            console.log('drawing')
            console.log(bodies)
