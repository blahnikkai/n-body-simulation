...
function main() {
    ...
    // moon ?!
    bodies.push(new Body(7.35e22, new Vector((400 + 92 - 6.1) * DIST_SCALE, 400 * DIST_SCALE), new Vector(0, -37983549.0706 - 1.35 * 2555031)))
    ...
    const render = () => {
        ...
        ctx.stroke()
        ctx.beginPath()
        ctx.arc(bodies[1].pos.x / DIST_SCALE, bodies[1].pos.y, 6.1, 0, 2 * Math.PI)
        ctx.stroke()
        for(let i = 0; i < bodies.length; ++i) {
            bodies[i].integrate()
            ctx.beginPath()
