...
const TIME_SCALE = 5
const FRAMERATE = 60
const VEL_ARROW_FACT = 1e-6
const ACC_ARROW_FACT = 1e-6
...
class Body {
    ...
    draw_vel(ctx) {
        ...
        ctx.lineTo(this.pos.x / DIST_SCALE + VEL_ARROW_FACT * this.vel.x, this.pos.y / DIST_SCALE + VEL_ARROW_FACT * this.vel.y)
        ctx.strokeStyle = 'red'
        ctx.stroke()
    ...
    draw_acc(ctx) {
        ctx.beginPath()
        ctx.moveTo(this.pos.x / DIST_SCALE, this.pos.y / DIST_SCALE)
        ctx.lineTo(this.pos.x / DIST_SCALE + ACC_ARROW_FACT * this.acc.x, this.pos.y / DIST_SCALE + ACC_ARROW_FACT * this.acc.y)
...
function main() {
    ...
    canvas.addEventListener('click', (e) => {
        ...
        if(!adding) {
            ...
            adding_body = new Body(1e31, new Vector(DIST_SCALE * x, DIST_SCALE * y), new Vector(0, 0))
        }
        else {
            const x = e.clientX - rect.left
            const y = e.clientY - rect.top
            const x_vel = x - adding_body.pos.x
            const y_vel = x - adding_body.pos.x
            // adding_body.vel = new Vector(x / VEL_ARROW_FACT, y / VEL_ARROW_FACT)
        }
    })
    const step = () => {
