...
export default class Body {
    ...
    draw_pos(ctx, screen_center) {
        ctx.beginPath()
        ctx.arc(this.pos.x / DIST_SCALE - screen_center.x, this.pos.y / DIST_SCALE - screen_center.y, 2, 0, 2 * Math.PI)
        ctx.fill()
    }

    draw_vel(ctx) {
        ctx.beginPath()
        ctx.moveTo(this.pos.x / DIST_SCALE - screen_center.x, this.pos.y / DDIST_SCALE - screen_center.y)
        ctx.lineTo((this.pos.x + this.vel.x) / DIST_SCALE  - screen_center.x, (this.pos.y + this.vel.y) / DIST_SCALE - screen_y) 
        ctx.strokeStyle = 'red'
        ctx.stroke()
