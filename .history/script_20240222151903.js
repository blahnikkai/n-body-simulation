...
function main() {
    ...
    let bodies = []
    // sun
    bodies.push(new Body(1.99e33, new Vector(150 * DIST_SCALE, 400 * DIST_SCALE), new Vector(0, 0)))
    // earth
    bodies.push(new Body(5.97e24, new Vector((150 + 92) * DIST_SCALE, 150 * DIST_SCALE), new Vector(0, -37983549.0706)))
    // moon ?!
    bodies.push(new Body(7.35e22, new Vector((150 + 92 + 3) * DIST_SCALE, 150 * DIST_SCALE), new Vector(0, -37983549.0706 + 3 * 1023000)))
    ...
    const render = () => {
        ...
        ctx.beginPath()
        ctx.arc(150, 150, 92, 0, 2 * Math.PI)
        ctx.stroke()
        for(let i = 0; i < bodies.length; ++i) {
            bodies[i].integrate()
            ctx.beginPath()
