...
function main() {
    ...
    let bodies = []
    // sun
    bodies.push(new Body(1.99e30, new Vector(150 * DIST_SCALE, 150 * DIST_SCALE), new Vector(0, 0)))
    // earth
    bodies.push(new Body(5.97e24, new Vector((150 + 92) * DIST_SCALE, 150 * DIST_SCALE), new Vector(0, -29784.8)))
    ...
    const render = () => {
        ...
        for(let i = 0; i < bodies.length; ++i) {
            bodies[i].integrate()
            ctx.beginPath()
            ctx.arc(bodies[i].pos.x / DIST_SCALE, bodies[i].pos.y, 10, 0, 2 * Math.PI)
            ctx.fill()
            console.log('drawing')
            console.log(bodies)
